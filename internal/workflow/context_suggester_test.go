...
func TestNewContextSuggester(t *testing.T) {
	...
	assert.Len(t, suggester.triggers, 8) // Should have 8 trigger types
}
...
func TestContextSuggester_AnalyzeContext(t *testing.T) {
	...
	problemType := types.ChunkTypeProblem
	storage.On("FindSimilar", mock.Anything, mock.AnythingOfType("string"), &problemType, mock.AnythingOfType("int")).Return(
		[]types.ConversationChunk{problemChunk}, nil)

	storage.On("Search", mock.Anything, mock.AnythingOfType("string"), mock.Anything, mock.AnythingOfType("int")).Return(
