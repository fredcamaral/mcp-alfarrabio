...
// EmbeddingGenerator interface for generating embeddings (duplicate from summarizer.go for independence)
...
// NewMemoryDecayManagerWithEmbeddings creates a new memory decay manager with intelligent LLM summarization
func NewMemoryDecayManagerWithEmbeddings(config *DecayConfig, store MemoryStore, embeddingGen EmbeddingGenerator) *MemoryDecayManager {
	if config == nil {
		config = DefaultDecayConfig()
	}

	// Use intelligent LLM summarizer with embeddings
	summarizer := NewLLMSummarizer(embeddingGen)
