	...
	tests := []struct {
		...
		},
		{
			...
			setupEnv: func() {
				...
				_ = os.MkdirAll(tmpDir, 0750)                        // Test directory creation - safe to ignore error
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if backupDir := os.Getenv("MCP_MEMORY_BACKUP_DIRECTORY"); backupDir != "" {
				_ = os.MkdirAll(backupDir, 0750)               // Test directory creation - safe to ignore error
				defer func() { _ = os.RemoveAll(backupDir) }() // Test cleanup
			}
			if auditDir := os.Getenv("MCP_MEMORY_AUDIT_DIRECTORY"); auditDir != "" {
				_ = os.MkdirAll(auditDir, 0750)               // Test directory creation - safe to ignore error
